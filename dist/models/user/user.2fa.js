...
module.exports = function mixUser2faModule() {
     ...
     */
    this.disable2fa = () => {
        return verifyProtectedAction('disable').then(() => socket.send('/2fa/disable')).then(() => {
    ...
    this.reissueBackupCodes = () => {
        return verifyProtectedAction('backupCodes').then(() => socket.send('/2fa/backup-codes/reissue')).then(res => res.backupCodes).tapCatch(err => {
            console.error(err);
    ...
    this._handle2faOnLogin = () => {
        return new Promise((resolve, reject) => {
            clientApp.create2FARequest('login', (code, trustDevice = false) => {
                ...
                socket.send('/noauth/2fa/authenticate', req).then(resp => {
    ...
    function verifyProtectedAction(type) {
        return new Promise((resolve, reject) => {
            clientApp.create2FARequest(type, code => {
                code = sanitizeCode(code); //eslint-disable-line
                const req = {
                    [code.length === 6 ? 'TOTPCode' : 'backupCode']: code
                };
                socket.send('/auth/2fa/verify', req).then(resolve).catch(reject);
            }, () => {

...
module.exports = function mixUserAuthModule() {
    this._authenticateConnection = () => {
        ...
        return this._loadAuthSalt().then(this._deriveKeys).then(this._getAuthToken).then(this._authenticateAuthToken).catch(e => {
            // eslint-disable-next-line default-case
            switch (e.code) {
                case errors.ServerError.codes.invalidDeviceToken:
                    console.log('Bad deviceToken, reauthenticating without one.');
                    return TinyDb.system.removeValue(`${this.username}:deviceToken`).then(this._authenticateConnection);
                ...
                case errors.ServerError.codes.twoFAAuthRequired:
                    ...
                    return this._handle2faOnLogin().catch(err => {
    ...
    this._deriveKeys = () => {
        ...
        return keys.deriveAccountKeys(this.username, this.passphrase, this.authSalt).then(keySet => {
    ...
    this._loadAuthSalt = () => {
        ...
        return socket.send('/noauth/auth-salt/get', { username: this.username }).then(response => {
            this.authSalt = new Uint8Array(response.authSalt);
        });
    };
    this._getAuthToken = () => {
        console.log('Requesting auth token.');
        return TinyDb.system.getValue(`${this.username}:deviceToken`).then(deviceTokenString => {
    ...
    this._authenticateAuthToken = data => {
        ...
        const decrypted = publicCrypto.decryptCompat(data.token, data.nonce, data.ephemeralServerPK, this.authKeys.secretKey);
    ...
    this._checkForPasscode = skipCache => {
        ...
        return TinyDb.system.getValue(`${this.username}:passcode`).then(passcodeSecretArray => {
    ...
    this._derivePassphraseFromPasscode = passcodeSecret => {
        ...
        return this._getAuthDataFromPasscode(this.passphrase, passcodeSecret).then(this.deserializeAuthData).catch(() => {
    ...
    this._getAuthDataFromPasscode = (passcode, passcodeSecret) => {
        return keys.deriveKeyFromPasscode(this.username, passcode).then(passcodeKey => secret.decryptString(passcodeSecret, passcodeKey)).then(authDataJSON => JSON.parse(authDataJSON));
    };
    ...
    this.setPasscode = passcode => {
        ...
        return keys.deriveKeyFromPasscode(this.username, passcode).then(passcodeKey => {
            ...
            return TinyDb.system.removeValue(`${this.username}:passcode:disabled`).catch(err => {
                if (err.message === 'Invalid tinydb key') {
                    return true;
                }
                return Promise.reject(err);
    ...
    this.validatePasscode = passcode => {
        ...
        return u._checkForPasscode().then(() => {
            return u.passphrase && u.passphrase !== passcode ? u.passphrase : Promise.reject(new Error('user.auth.js: passcode is not valid'));
        });
    };
     ...
     */
    this.signout = () => {
        return socket.send('/auth/signout').timeout(3000).catch(err => {
            // not a show stopper
            console.error(err);
        });
    };

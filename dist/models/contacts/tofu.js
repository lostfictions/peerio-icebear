 ...
 * @public
 */
let Tofu = (_temp = _class = class Tofu extends Keg {
    constructor(db) {
        super(null, 'tofu', db);
    }
    ...
    detectTampering(payload) {
        ...
        if (payload.username !== this.username) {
            throw new AntiTamperError('Tofu keg inner and outer username mismatch. ' + `Inner: ${payload.username}. Outer: ${this.username}`);
        }
    }
    /**
...
}, _class.cache = null, _class.preCacheRequests = [], _temp);

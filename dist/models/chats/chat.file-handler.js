 ...
 * @public
 */
let ChatFileHandler = class ChatFileHandler {
    constructor(chat) {
        this.shareQueue = new TaskQueue(1, 2000);
    ...
    getRecentFiles() {
        return retryUntilSuccess(() => {
            return socket.send('/auth/kegs/db/files/latest', { kegDbId: this.chat.id, count: config.chat.recentFilesDisplayLimit }).then(res => {
                const ids = [];
                res.forEach(raw => {

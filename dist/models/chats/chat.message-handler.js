...
let ChatMessageHandler = class ChatMessageHandler {
    constructor(chat) {
        ...
        this.onMessageDigestUpdate = _.throttle(() => {
        ...
        this._reactionsToDispose.push(reaction(() => socket.authenticated && clientApp.isFocused && clientApp.isInChatsView && this.chat.active, userIsReading => {
            if (userIsReading) {
                this.markAllAsSeen();
                this.removeMaker();
    ...
    } // todo: make this observable in Chat
    ...
    getPage(pagingUp = true, startingKegId = null) {
        if (!this.chat.initialPageLoaded || pagingUp && this.chat.loadingTopPage || !pagingUp && this.chat.loadingBottomPage) {
            return;
        }
        ...
        retryUntilSuccess(() => socket.send('/auth/kegs/db/list-ext', {
            ...
            console.debug('Received page', pagingUp ? 'UP' : 'DOWN', pagingUp && this.chat._cancelTopPageLoad || !pagingUp && this.chat._cancelBottomPageLoad ? 'and discarded' : '');
            if (pagingUp) {

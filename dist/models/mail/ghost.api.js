...
ghostAPI.deriveKeys = function (ghost) {
    return keys.deriveEphemeralKeys(cryptoUtil.hexToBytes(ghost.ghostId), ghost.passphrase).then(kp => {
...
ghostAPI.encrypt = function (ghost, user, serializedGhost) {
    ...
    try {
        ...
        return sign.signDetached(res.body, user.signKeys.secretKey).then(cryptoUtil.bytesToB64).then(signature => {
